...
Подсистема не ведёт задачи, планы, трудозатраты, дефекты и тестовые прогоны — она их читает. Фактические данные принадлежат [Task Tracker](../../task-tracker/README.md), проекты, плановые рамки, ресурсы и бюджетные корректировки — [Planning System](../../planning-system/README.md), классификаторы — [Reference Manager](../../reference-manager/README.md). Данные тестирования поступают из отдельной системы управления тестированием, контракт с которой будет согласован после появления её документации. Reports собирает данные в витрину отчётности, считает показатели и представляет в виде таблиц, графиков и выгрузок.

### 1.2. Место в системе

Пользователь входит через [Keycloak](../../keycloak/README.md) и обращается к Reports с токеном. Reports получает факт из Task Tracker, плановые данные из Planning System, классификаторы из Reference Manager и результаты тестирования из будущей системы тестирования. Подсистема хранит витрину в собственной схеме БД по правилам [Core](../../core/README.md), а обмен событиями, объектное хранение выгрузок и развёртывание обеспечивает [Infra](../../infra/README.md).
...
flowchart LR
    ...
    TS["Система управления тестированием<br/>(контракт уточняется)"]
    KF["Kafka"]
    S3["S3"]
    ...
    PS -->|"Проекты, планы, ресурсы, календарь, бюджет"| ING
    RM -->|"Классификаторы, роли, компетенции"| ING
    TS -.->|"Тест-планы и результаты"| ING
    API -->|"Файл выгрузки"| S3
    S3 -->|"Ссылка на файл"| U
    API -->|"Отчёт и уведомление о готовности"| U
```
...
Изменения задач, трудозатрат, статусов, итераций и дефектов поступают из Task Tracker событиями через Kafka после согласования межкомандного контракта. Подсистема применяет события к витрине идемпотентно: повторная доставка одного события не приводит к повторному учёту часов или дефектов. REST используется для первичного наполнения и сверки по тому же согласованному контракту. Плановые данные — проекты, сроки, ресурсные назначения, календарь и бюджетные корректировки — запрашиваются из Planning System по согласованному API; события будут добавлены только после согласования контракта. Классификаторы, роли, грейды и компетенции запрашиваются из Reference Manager по REST и могут кэшироваться в Reports, но не изменяются ею. Тест-планы и результаты будут поступать из системы управления тестированием после согласования её контракта. По расписанию выполняется сверка витрины с источниками: расхождения устраняются, факт и результат сверки фиксируются. После успешного обновления записывается время актуальности данных, которое показывается в каждом отчёте. При недоступности источника подсистема продолжает отдавать отчёты по последним полученным данным, явно предупреждая пользователя об устаревании.

```mermaid
flowchart TD
    A["Изменение данных в Task Tracker"] --> B["Публикация согласованного события в Kafka"]
    ...
    M --> G
    N --> G
    O["Запрос справочников из Reference Manager"] --> F
    P["Запрос плановых данных из Planning System"] --> F
    Q["Получение данных тестирования после согласования контракта"] -.-> F
```
...
- **FR-CALC-08.** Подсистема не должна изменять данные источников; все её операции с данными Task Tracker, Planning System, Reference Manager и системы управления тестированием должны быть только чтением.
...
| --- | --- | --- | --- | --- |
| Task Tracker | Задачи и их атрибуты, виды задач, статусы и история их изменений, приоритеты, исполнители, сроки, плановая и фактическая трудоёмкость, записи трудозатрат, итерации, иерархия задач, дефекты | События через Kafka и REST для первичного наполнения и сверки — после согласования межкомандного контракта | Чтение | Критичная: без неё отчёты невозможны |
| Planning System | Проекты, версии и базовые линии планов, эпики и фичи, плановые сроки, ресурсная потребность, доступность и назначения сотрудников, календарь, прогнозы и бюджетные корректировки | REST для чтения и сверки; события — после согласования контракта | Чтение | Критичная для R-02, R-04, R-06, R-09 и R-11 |
| Reference Manager | Классификаторы для разрезов отчётов: роли, грейды, компетенции, приоритеты, серьёзность и другие согласованные справочники | REST с кэшированием на стороне отчётов; Reports не изменяет справочники | Чтение | Высокая: без неё недоступны согласованные разрезы отчётов |
...
| Система управления тестированием | Тест-планы, тест-кейсы, прогоны, результаты, связь с дефектами и релизами/итерациями | Контракт будет согласован после публикации документации команды тестирования | Чтение | Критичная для R-07 и R-09 |
...
| Проекты, плановые сроки, версии и базовые линии плана | R-01, R-02, R-04, R-06, R-08, R-09, R-11 | Planning System | Есть в vision Planning System; формат выдачи Reports требует согласования |
| Эпики и фичи, связь с задачами | R-03, R-04 | Planning System и Task Tracker | Требуется согласовать идентификаторы и связь с задачами |
...
| Тест-планы, тест-кейсы, прогоны и их результаты | R-07, R-09 | Система управления тестированием | Точный контракт ожидает публикации документации команды тестирования |
| Ёмкость сотрудника: норма рабочего времени, календарь, отсутствия, занятость в проектах | R-02, R-06 | Planning System | Есть в vision Planning System; формат выдачи Reports требует согласования |
| Роли и компетенции сотрудников | R-02 | Reference Manager | Требует согласования |
| Бюджет работ: плановый объём часов и при необходимости стоимость часа | R-06 | Planning System | Есть в vision Planning System; формула и детализация требуют согласования |
...
flowchart LR
    ...
    PS["Planning System"] -->|"REST: проекты, планы, ресурсы, календарь, бюджет"| ING
    RM["Reference Manager"] -->|"REST: классификаторы, роли, грейды, компетенции"| ING
    TS["Система управления тестированием"] -.->|"Тест-планы, прогоны, результаты; контракт уточняется"| ING
...
- Отчёты R-07 и R-09 зависят от согласования контракта с системой управления тестированием; до этого можно зафиксировать только состав требуемых данных.
- Отчёты R-02 и R-06 зависят от согласования формата выдачи Planning System для ресурсной потребности, доступности сотрудников и бюджетных корректировок.
- Разрезы по проекту и эпику зависят от согласования идентификаторов Planning System с задачами Task Tracker.
...
1. Какой API или набор событий Planning System предоставляет Reports для проектов, базовых линий, сроков, ресурсов, календаря и бюджета?
2. Как связываются идентификаторы проектов, эпиков и фич из Planning System с задачами и итерациями Task Tracker?
3. Какой контракт предоставит система управления тестированием: сущности, идентификаторы, REST API или события, периодичность и правила повторной доставки?
...
10. В каком формате и с какой периодичностью Planning System передаёт в Reports календарь рабочего времени, отсутствия и нормы загрузки сотрудников?
