...
Актуализировано: 22 сентября 2026 года. Документ описывает целевую архитектуру учебного стенда и показатели назначения инфраструктуры. Диаграммы не означают, что компоненты уже развёрнуты. Это единый vision команды инфраструктуры; бизнес-требования остаются у команд подсистем.
...
## 10. Показатели назначения

Инфраструктура не задаёт функциональные требования прикладным командам: состав сервисов, контракты и границы ответственности описаны в разделах выше. Показатели назначения измеряют, насколько общее окружение выполняет свою роль — общие сервисы доступны, данные сохраняются, выпуск воспроизводится. Значения относятся к учебному одновузловому стенду в согласованные часы работы. Отказ Docker-хоста целиком, плановые работы и недоступность приложений команд в эти показатели не входят и не маскируются обещанием высокой доступности.

| ID | Показатель | Целевое значение | Как проверяется |
| --- | --- | --- | --- |
| PN-01 | Доступность общих сервисов | ≥ 99,5% за календарный месяц | Доля успешных периодических проверок готовности Keycloak, БД Keycloak, MinIO и Kafka. Плановые окна вычитаются из знаменателя. |
| PN-02 | Аптайм | Суммарный незапланированный простой каждого общего сервиса ≤ 3,5 ч за месяц | Интервалы, когда healthcheck не проходит вне окна обслуживания. Согласовано с PN-01. |
| PN-03 | Время восстановления (RTO) | ≤ 1 ч | От фиксации потери сервиса или тома до прохождения готовности после восстановления из резервной копии на том же хосте. |
| PN-04 | Допустимая потеря данных (RPO) | ≤ 24 ч | Максимальный интервал между успешными копиями БД Keycloak, объектов MinIO и журнала Kafka. |
| PN-05 | Время отклика инфраструктурных точек входа | p95: выдача токена и JWKS Keycloak ≤ 200 мс; PutObject/GetObject объекта до 1 МБ ≤ 300 мс | Синтетические запросы со стенда при нагрузке в пределах PN-07. |
| PN-06 | Время холодного запуска | ≤ 5 мин | От запуска инфраструктурных сервисов Compose до `service_healthy` у всех из них. |
| PN-07 | Ёмкость учебного стенда | Одновременно работают четыре команды и общие сервисы; ориентир нагрузки — до 100 запросов/с к Keycloak без отказа готовности | Нагрузочная проверка перед демонстрацией. |
| PN-08 | Воспроизводимость выпуска | Повтор развёртывания из тех же digest и коммита конфигурации даёт тот же состав; непрошедшая сборка или готовность не считается выпуском | Журнал GitHub Actions и отчёт о развёртывании. |
| PN-09 | Успешность восстановления | Последняя резервная копия восстанавливается на отдельном окружении без потери согласованности конфигурации | Демонстрация восстановления до выпуска стенда. |

Целевые показатели прикладных API — время ответа справочников, лаг витрины отчётов — остаются у команд подсистем. Нагрузка сверх PN-07, квоты бакетов и сроки хранения объектов согласовываются отдельно и не подменяют таблицу выше.
